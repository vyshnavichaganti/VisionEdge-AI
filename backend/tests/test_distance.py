...
def test_distance_estimator_person():
    ...
    # Formula: (1.70 * 650.0) / 200.0 = 5.525 -> round 5.5
    assert dist_m == 5.5
    assert "≈" in dist_disp
...
def test_distance_estimator_cell_phone():
    ...
    # Formula: (0.15 * 650.0) / 120.0 = 0.8125 -> round 0.8
    assert dist_m == 0.8
    assert dist_disp == "≈ 0.8 m"
    assert dist_qual == "approximate"

def test_distance_estimator_unknown_class():
    estimator = DistanceEstimator()
    # Unknown class should fall back to default reference height (0.50m) without throwing
    dist_m, dist_disp, dist_qual = estimator.estimate("unidentified_ufo", (50, 100, 150, 300), img_height=480)
    assert dist_m > 0
    assert "m" in dist_disp

def test_distance_estimator_zero_width_or_height():
    estimator = DistanceEstimator()
    # Zero width or zero height should return safe unavailable fallback instead of division by zero
    dist_m, dist_disp, dist_qual = estimator.estimate("person", (100, 100, 100, 100), img_height=480)
    assert dist_m == 0.0
    assert dist_disp == "N/A"
    assert dist_qual == "unavailable"

def test_distance_estimator_clamping():
    estimator = DistanceEstimator()
    # Extremely small height -> clamp to 50.0m max
    dist_m, _, _ = estimator.estimate("person", (0, 0, 10, 2), img_height=480)
    assert dist_m <= 50.0
